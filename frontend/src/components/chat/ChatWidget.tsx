...
export function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", text: "Pregunta sobre alertas, sucursales criticas o pedidos por revisar." },
  ]);
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-2 border-b border-slate-200 px-4 py-3">
        ...
        <h2 className="text-sm font-semibold text-slate-950">Chat con datos</h2>
      </div>
      <div className="max-h-72 space-y-3 overflow-y-auto bg-slate-50 px-4 py-4">
        ...
        {mutation.isPending && <p className="text-sm text-slate-500">Analizando datos...</p>}
      </div>
